              ...
              <v-container>
                <v-row>
                  ...
                  <v-col
                    ...
                    sm="6"
                    md="4">
                    <v-textarea :disabled="true"
                      v-model="editedItem.occupationArea"
                      label="Área de atuação"></v-textarea>
                  </v-col>
                  <v-col
                    cols="12"
                    sm="6"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions('account', ['getAllCompanies', 'updateStatus']),
      initialize () {
        this.getList()
      },
      ...
      closeApprove () {
        ...
        var company = this.companies.find(element => element.id == id);

        if(company.active != approve) {
          this.updateStatus({ id, approve }).then(() => {
              company.active = approve;
     ...
     getList() {
        ...
        this.getAllCompanies({ page, size }).then(response => {
